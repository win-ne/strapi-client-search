...
import searchData from '@/public/search_data.json'
import searchIndexData from '@/public/search_index.json'
...
function Search() {
    const searchIndex = Fuse.parseIndex(searchIndexData)
    const options = { keys: ["name", "description", "link", "type"], includeMatches: true, useExtendedSearch: true }
    const fuse = new Fuse(searchData, options, searchIndex)
    ...
    return <div className="flex p-8 pb-20 gap-8 sm:p-20 font-[family-name:var(--font-geist-sans)] flex-col">
        ...
        <input type="text" className="rounded-lg bg-white/15 h-10 text-white py-2 px-4 hover:border hover:border-white/25 active:border active:border-white/25 focus:border focus:border-white/25" onChange={e => setSearchTerm(e.target.value)} />
            ...
            {results.map(res => {
                ...
                return <Link
                    ...
                    <div className='flex flex-col justify-start items-start'>
                        <div className='bg-white text-black rounded-lg p-1 text-xs shrink font-semibold mb-2'>{hit.type}</div>
